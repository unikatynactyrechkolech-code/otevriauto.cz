...
export default function LocationsDirectory({ currentSlug }: LocationsDirectoryProps) {
  const chipClass = (active: boolean) =>
    `rounded-none px-1.5 py-0.5 text-[11px] font-semibold transition sm:px-2.5 sm:py-1 sm:text-sm ${
      active
        ? "bg-brand text-black"
        : "bg-white/10 text-white hover:bg-brand/25 hover:text-white"
    }`;
  const regionClass = "scroll-mt-20 rounded-none bg-white/[0.04] p-2 sm:p-6";
        ...
        <div id={`lokality-${prague.id}`} className={`mt-8 ${regionClass}`}>
          ...
          <div className="mt-2 grid grid-cols-2 gap-1.5 sm:mt-4 sm:gap-3 lg:grid-cols-4">
            {prague.locations.map((location) => (
              <div key={location.slug} className="rounded-none bg-white/[0.05] p-1.5 sm:p-3">
                <Link
                  href={locationPath(location)}
                  className={`block rounded-none py-1.5 text-center font-heading text-sm font-bold transition sm:py-2.5 sm:text-base ${
          ...
          <div className="mt-2 grid grid-cols-2 gap-1.5 sm:mt-4 sm:gap-3 lg:grid-cols-3">
            {districts.map((district) => (
              <div
                ...
                className="scroll-mt-20 rounded-none bg-white/[0.05] p-1.5 sm:p-3"
              >
                ...
                <div className="mt-1.5 flex flex-wrap gap-1 sm:mt-2 sm:gap-1.5">
                  ...
                  {district.otherTowns.map((town) => (
                    <span
                      key={town}
                      className="rounded-none border border-white/40 px-1.5 py-0.5 text-[11px] text-white sm:px-2.5 sm:py-1 sm:text-sm"
                    >
                      {town}
                    </span>
                  ))}
